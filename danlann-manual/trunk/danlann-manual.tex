...
Danlann supports displaying Exif data on photo gallery pages.

By default, Danlann displays following Exif data
\begin{itemize}
...
Listing~\ref{conf:exif} contains example of configuration, which displays
only Exif data like timestamp, exposure time and aperture.

Exif header names can be determined using \texttt{exiv2} command, which is
used by Danlann to get photo data.
...
\hline
files          & \verb+css js+ & list of files separated by space to be copied from library paths to gallery output directory  & \\
\hline
...
\hline
libpath        &  & list of directories separated by colon, directories contain additional gallery files  & \\
\hline
...
\hline
\end{longtable}
%%conf.get('template', 'css')
%%conf.get('template', 'copyright')
%%
